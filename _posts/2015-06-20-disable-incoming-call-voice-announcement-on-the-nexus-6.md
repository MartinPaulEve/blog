---
archive: https://wayback.archive-it.org/22123/20231101171300/https://eve.gd/2015/06/20/disable-incoming-call-voice-announcement-on-the-nexus-6
categories:
- Technology
date: 2015-06-20
doi: https://doi.org/10.59348/mebpy-kcr46
roguescholar: https://rogue-scholar.org/records/phpag-5e131
atproto: at://did:plc:hnpt7ns2lecdujegbi6qkqqm/site.standard.document/3mq7mcmsf5x2u
image:
  feature: geek.png
layout: post
ogImage: images/geek.png
published: true
tags: []
title: Disable "incoming call" voice announcement on the Nexus 6
atUri: "at://did:plc:hnpt7ns2lecdujegbi6qkqqm/site.standard.document/3mq7mcmsf5x2u"
kcworks: https://works.hcommons.org/records/c40r6-zxj84
biron: https://eprints.bbk.ac.uk/id/eprint/17419/
---

A quick post to help anyone else, as I couldn't find this info easily on the web. In the middle of the night, I was awoken by my Nexus 6 announcing an "Incoming call from unknown number", despite the fact I had priority mode on and it shouldn't have said anything.

In any case, this seems to fix it:

1. Go to Settings, Accessibility, and click on "Talkback", even if it says Talkback is "off".
2. Click on "Settings" under Talkback (at the top right)
3. Disable "Speak caller ID"